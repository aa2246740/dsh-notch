...
work=$(mktemp -d "${TMPDIR:-/tmp}/notch-edge.XXXXXX")
trap 'rm -rf "$work"' EXIT
mkdir "$work/Sources"
cp -R "$base/Sources/Resources" "$work/Sources/Resources"
cp "$base/Package.swift" "$work/Package.swift"
for name in IdleRobot NotchMarkdown RootView Panel Client StatusOrbit EdgeDock; do
  cp "$base/Sources/$name.swift" "$work/Sources/"
done
cp "$base/Tests/EdgeDockProbe.swift" "$work/Sources/main.swift"
