...
| 6 | 令牌文件权限（`0600` 在 Windows 是空操作） | 低 | **接受现状，不加显式 ACL**（判定证据见 §15 Phase 7）：`runtime.json` 位于 `%USERPROFILE%\.dsh\dsh-notch\`，实测（`icacls`）其 ACL 是从 profile 继承的「本人 + SYSTEM + Administrators」，也就是说 0o600 想表达的"只有本人可读"在 NTFS 上**本来就成立**；而它想挡的"同一用户下的其他进程"无论加不加 ACL 都挡不住（同一 SID 的进程可直接读该文件）。显式 `icacls` 收紧只会引入一个"收紧失败即安装中断"的新失败点，却换不来额外的边界 |
